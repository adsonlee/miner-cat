...
const App: React.FC = () => {
  ...
  return (
    <div className="h-[100dvh] w-full bg-zinc-900 flex items-center justify-center p-0 md:p-4 relative overflow-hidden font-sans select-none">
      ...
      {/* === 游戏机容器 === */}
      {/* 适配 md:max-w-[600px] 更适合竖屏 */}
      <div className="relative z-10 w-full h-full md:h-auto md:max-w-[600px] bg-slate-800 md:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-0 md:border-8 border-slate-700 ring-0 md:ring-1 ring-white/10 flex flex-col overflow-hidden">
        ...
        <div className="bg-slate-900/90 text-white px-4 py-2 md:px-6 md:py-4 flex justify-between items-center border-b-4 border-slate-700 shadow-md z-20 shrink-0">
           <div className="flex flex-col items-center">
              ...
              <div className="bg-black shadow-inner px-2 md:px-4 py-1 md:py-2 rounded-lg border-2 border-slate-700 min-w-[70px] md:min-w-[100px]">
        ...
        <div className="relative flex-1 w-full bg-black shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] flex items-center justify-center overflow-hidden">
            {/* 关键修改：aspect-[3/4] 竖屏比例 */}
            <div className="relative aspect-[3/4] w-full h-auto max-w-full max-h-full m-auto shadow-2xl">
              <GameCanvas
              ...
              {gameState === GameState.MENU && (
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white backdrop-blur-sm z-30">
                  ...
                  <div className="flex flex-col gap-4 w-64">
                    <button onClick={handleStartClick} className="flex items-center justify-center gap-2 px-6 py-4 bg-green-600 hover:bg-green-500 rounded-xl font-bold text-xl shadow-[0_4px_0_rgb(21,128,61)] hover:shadow-[0_2px_0_rgb(21,128,61)] hover:translate-y-[2px] transition-all">
                      <Play size={24} fill="currentColor" /> Start Game
                    </button>
                    <button onClick={() => setGameState(GameState.LEADERBOARD)} className="flex items-center justify-center gap-2 px-6 py-4 bg-amber-600 hover:bg-amber-500 rounded-xl font-bold text-xl shadow-[0_4px_0_rgb(180,83,9)] hover:shadow-[0_2px_0_rgb(180,83,9)] hover:translate-y-[2px] transition-all">
              ...
              {gameState === GameState.INPUT_NAME && (
                <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center text-white backdrop-blur-md z-30 p-4">
                  <div className="bg-slate-800 p-6 md:p-8 rounded-2xl border-4 border-slate-600 shadow-2xl text-center max-w-sm w-full">
                    ...
                    <input type="text" value={tempNickname} onChange={(e) => setTempNickname(e.target.value)} placeholder="Player Name" maxLength={8} className="w-full bg-black text-center text-xl md:text-2xl font-mono text-white p-3 md:p-4 rounded-lg border-2 border-slate-500 focus:border-amber-500 outline-none mb-6 uppercase" autoFocus />
              ...
              {gameState === GameState.LEADERBOARD && (
                <div className="absolute inset-0 bg-slate-900 z-30 flex flex-col">
                  <div className="p-4 md:p-6 flex items-center justify-between bg-slate-800 border-b border-slate-700 shrink-0">
                    <button onClick={() => setGameState(GameState.MENU)} className="text-slate-400 hover:text-white flex items-center gap-2 font-bold text-sm md:text-base"><ArrowLeft size={20} /> BACK</button>
                    <h2 className="text-lg md:text-2xl font-black text-amber-500 flex items-center gap-2"><Trophy size={20} className="text-amber-400" /> HALL OF FAME</h2>
                ...
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white backdrop-blur-sm z-30 p-4">
                  ...
                  <div className="flex flex-col gap-3 w-64">
                      {score >= INITIAL_LEVEL_CONFIG.targetScore + (level - 1) * 200 ? (
                         <>
                           <button onClick={nextLevel} className="w-full px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-xl shadow-[0_4px_0_rgb(37,99,235)] hover:shadow-[0_2px_0_rgb(37,99,235)] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2">Next Level <Play size={20} fill="currentColor"/></button>
                           <button onClick={resetGame} className="w-full px-6 py-3 bg-slate-600 hover:bg-slate-500 rounded-xl font-bold text-lg shadow-[0_4px_0_rgb(71,85,105)] hover:shadow-[0_2px_0_rgb(71,85,105)] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2"><Home size={20} /> Main Menu</button>
                         </>
                      ) : (
                        <div className="flex flex-col items-center w-full gap-3">
                           <p className="text-red-400 mb-2 font-bold">Target not reached!</p>
                           <button onClick={() => setGameState(GameState.GAME_OVER)} className="w-full px-6 py-3 bg-red-600 hover:bg-red-500 rounded-xl font-bold text-xl shadow-[0_4px_0_rgb(153,27,27)] hover:shadow-[0_2px_0_rgb(153,27,27)] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2"><Skull size={20} /> Finish Game</button>
                        </div>
                      )}
                  </div>
                </div>
              ...
              {gameState === GameState.GAME_OVER && (
                 <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center text-white backdrop-blur-md z-30 p-4">
                    ...
                    <div className="flex gap-4">
                       <button onClick={resetGame} className="px-8 py-3 bg-white text-black hover:bg-gray-200 rounded-full font-bold text-xl shadow-lg hover:scale-105 transition-all">Back to Menu</button>
                    </div>
                 </div>
              )}
        ...
        <div className="bg-slate-800 px-4 py-2 md:px-6 md:py-3 flex justify-between items-center border-t-4 border-slate-700 shrink-0">
           <div className="flex items-center gap-2">
              ...
              <span className="text-slate-400 font-bold uppercase tracking-widest text-xs md:text-sm">{nickname ? `${nickname}` : 'READY'}</span>
           </div>
           <div className="flex items-center gap-2">
               <span className="text-amber-500 font-bold uppercase tracking-widest font-mono text-xs md:text-sm border border-amber-500/30 px-2 py-0.5 rounded bg-amber-900/20">LVL {level}</span>
           </div>
        </div>
